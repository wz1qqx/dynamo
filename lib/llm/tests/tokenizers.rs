...
/// Encode→decode roundtrip for multi-byte UTF-8 content (CJK, emoji, mixed).
/// The mock tiktoken model has all 256 single-byte tokens, so non-ASCII text
/// gets encoded via byte-fallback. This confirms from_utf8_lossy() in the
/// decode path doesn't corrupt data when all bytes are present.
#[test]
fn test_tiktoken_roundtrip_multibyte_utf8() {
...
/// Encode→decode roundtrip via DecodeStream (incremental/streaming path).
/// This is the path that actually triggered the original crash: tokens are
/// decoded one at a time through a sliding window, and byte-fallback tokens
/// produce incomplete UTF-8 sequences at intermediate steps.
#[test]
fn test_tiktoken_roundtrip_multibyte_decode_stream() {
    let path = mock_tiktoken_model_path();
    let tokenizer =
        TikTokenTokenizer::from_file_auto(&path).expect("Failed to load tiktoken tokenizer");
    ...
    for &(prompt, output_text) in test_cases {
        let prompt_encoding = shared_tokenizer
            .encode(prompt)
            .unwrap_or_else(|e| panic!("Failed to encode prompt '{prompt}': {e}"));
        ...
        assert_eq!(
            reassembled, output_text,
            "DecodeStream roundtrip failed for: '{output_text}'"
        );
    }
}

/// Encode→decode roundtrip via Sequence::append_token_id (the other streaming path).
/// This exercises the same byte-fallback handling through the Sequence API.
#[test]
fn test_tiktoken_roundtrip_multibyte_sequence() {
    let path = mock_tiktoken_model_path();
    let tokenizer =
        TikTokenTokenizer::from_file_auto(&path).expect("Failed to load tiktoken tokenizer");
