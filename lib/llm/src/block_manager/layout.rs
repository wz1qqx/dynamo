...
pub mod tests {
    ...
    mod mla_outer_dim_1_tests {
        ...
        fn test_mla_config_validates() {
            ...
            assert!(config.validate().is_ok(), "outer_dim=1 should be valid");
        }

        #[test]
        fn test_mla_config_outer_dim_3_invalid() {
            let config = LayoutConfig::builder()
                .num_blocks(MLA_NUM_BLOCKS)
                .num_layers(MLA_NUM_LAYERS)
                .outer_dim(3)
            ...
            assert!(
                ...
                "outer_dim=3 should fail validation"
            );
        }
    ...
    // ============================================================================
    // layout_separate_auto DETECTION TESTS
    // ============================================================================

    mod layout_auto_detection_tests {
        use super::*;

        #[test]
        fn test_auto_detect_block_contiguous() {
            // shape[0] >= num_device_blocks → block contiguous (outer_contiguous=false)
            let shape = vec![1024, 2, 64, 128];
            let result = LayoutType::layer_separate_auto(&shape, 1024).unwrap();
            assert_eq!(
                result,
                LayoutType::LayerSeparate {
                    outer_contiguous: false
        ...
        #[test]
        fn test_auto_detect_outer_contiguous() {
            // shape[0] < num_device_blocks, shape[1] >= num_device_blocks → outer contiguous
            let shape = vec![2, 1024, 64, 128];
            let result = LayoutType::layer_separate_auto(&shape, 1024).unwrap();
            assert_eq!(
                result,
                LayoutType::LayerSeparate {
        ...
        #[test]
        fn test_auto_detect_unsupported_shape() {
            // Neither dimension >= num_device_blocks → error
            let shape = vec![4, 8, 64, 128];
            let result = LayoutType::layer_separate_auto(&shape, 1024);
            assert!(result.is_err());
        }

        #[test]
        fn test_auto_detect_mla_3d_tensor() {
            // MLA-style 3D tensor: [num_blocks, page_size, latent_dim]
            // shape[0] = num_blocks → block contiguous
            let shape = vec![12212, 64, 576];
            let result = LayoutType::layer_separate_auto(&shape, 12212).unwrap();
            assert_eq!(
                result,
                LayoutType::LayerSeparate {
                    outer_contiguous: false
        ...
        #[test]
        fn test_auto_detect_standard_4d_tensor() {
            // Standard 4D tensor: [num_blocks, 2, page_size, head_dim]
            let shape = vec![8000, 2, 64, 128];
            let result = LayoutType::layer_separate_auto(&shape, 8000).unwrap();
            assert_eq!(
                result,
                LayoutType::LayerSeparate {
                    outer_contiguous: false
